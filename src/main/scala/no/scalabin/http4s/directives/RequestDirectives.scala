...
trait RequestDirectives[F[+ _]] {
  ...
  implicit def MethodDirective(M: Method)(implicit eq: Eq[Method], sync: Monad[F]): Directive[F, Response[F], Method] =
    when[F, Method] { case req if eq.eqv(M, req.method) => M } orElse sync.pure(Response[F](Status.MethodNotAllowed))

  implicit def MethodsDirective(M: MethodConcat)(implicit sync: Monad[F]): Directive[F, Response[F], Method] =
...
trait RequestOps {
  ...
  def header[F[+ _]: Monad, KEY <: HeaderKey](key: KEY): Directive[F, Nothing, Option[key.HeaderT]] =
    headers.map(_.get(key.name)).map(_.flatMap(h => key.matchHeader(h)))
  def headerOrElse[F[+ _]: Monad, KEY <: HeaderKey, L](key: KEY, f: => F[L]): Directive[F, L, key.HeaderT] =
  ...
  def headerOrElse[F[+ _]: Monad, L](key: String, f: => F[L]): Directive[F, L, Header] =
    header(key).flatMap(opt => Directive.getOrElse(opt, f))

  def cookies[F[+ _]: Monad]: Directive[F, Nothing, Option[NonEmptyList[Cookie]]] =
  ...
  def cookie[F[+ _]: Monad](name: String): Directive[F, Nothing, Option[Cookie]] =
  ...
  def bodyAs[F[+ _]: Monad, A](implicit dec: EntityDecoder[F, A]): Directive[F, Nothing, A] =
